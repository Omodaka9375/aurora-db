var AuroraDB=(function(){'use strict';const{Peer}=window.peerjs||window;class SecureDiscoveryLayer{constructor(){this.encryptionKey=null;this.peerCertificates=new Map();this.trustedNetworks=new Set()}
async initializeSecurity(config){if(config.passphrase){this.encryptionKey=await this.deriveKey(config.passphrase)}
if(config.trustedPeers){for(const[peerId,cert]of Object.entries(config.trustedPeers)){this.peerCertificates.set(peerId,cert)}}
if(config.trustedNetworks){config.trustedNetworks.forEach(network=>this.trustedNetworks.add(network))}}
async deriveKey(passphrase){const encoder=new TextEncoder();const keyMaterial=await crypto.subtle.importKey('raw',encoder.encode(passphrase),{name:'PBKDF2'},!1,['deriveKey']);return crypto.subtle.deriveKey({name:'PBKDF2',salt:encoder.encode('auroradb-salt'),iterations:100000,hash:'SHA-256'},keyMaterial,{name:'AES-GCM',length:256},!1,['encrypt','decrypt'])}
async encryptDiscoveryData(data){if(!this.encryptionKey)return data;const iv=crypto.getRandomValues(new Uint8Array(12));const encodedData=new TextEncoder().encode(JSON.stringify(data));const encrypted=await crypto.subtle.encrypt({name:'AES-GCM',iv},this.encryptionKey,encodedData);return{encrypted:!0,iv:Array.from(iv),data:Array.from(new Uint8Array(encrypted))}}
async decryptDiscoveryData(encryptedData){if(!encryptedData.encrypted||!this.encryptionKey){return encryptedData}
try{const iv=new Uint8Array(encryptedData.iv);const data=new Uint8Array(encryptedData.data);const decrypted=await crypto.subtle.decrypt({name:'AES-GCM',iv},this.encryptionKey,data);return JSON.parse(new TextDecoder().decode(decrypted))}catch(error){console.warn('Failed to decrypt discovery data:',error);return null}}
validatePeer(peerId,discoveryMethod){return peerId&&peerId.length>8&&this.trustedNetworks.has(discoveryMethod)}}
class BitTorrentDHTAdapter extends EventTarget{constructor(config){super();this.config=config;this.namespace=config.namespace||'auroradb';this.infoHash=this.generateInfoHash();this.isConnected=!1;this.peers=new Map()}
generateInfoHash(){let hash=0;const str=this.namespace;for(let i=0;i<str.length;i++){const char=str.charCodeAt(i);hash=((hash<<5)-hash)+char;hash=hash&hash}
return Math.abs(hash).toString(16)}
async connect(){try{console.log(`🔗 Connecting to BitTorrent DHT with namespace: ${this.namespace}`);setTimeout(()=>{this.isConnected=!0;this.simulatePeerDiscovery();this.dispatchEvent(new CustomEvent('connected'))},1000);return!0}catch(error){console.error('DHT connection failed:',error);return!1}}
simulatePeerDiscovery(){setInterval(()=>{if(Math.random()>0.7){const peerId=`dht-peer-${Date.now()}-${Math.random().toString(36).substr(2, 8)}`;this.dispatchEvent(new CustomEvent('peer-discovered',{detail:{peerId,discoveryMethod:'bittorrent-dht',endpoints:['webrtc'],metadata:{infoHash:this.infoHash,discovered:Date.now()}}}))}},10000)}
async disconnect(){this.isConnected=!1;this.peers.clear()}}
class MQTTDiscoveryAdapter extends EventTarget{constructor(config){super();this.config=config;this.brokerUrl=config.brokerUrl||'wss://test.mosquitto.org:8081/mqtt';this.namespace=config.namespace||'auroradb';this.topic=`auroradb/${this.namespace}/discovery`;this.deviceId=`aurora-${Date.now()}-${Math.random().toString(36).substr(2, 8)}`;this.isConnected=!1}
async connect(){try{console.log(`🔗 Connecting to MQTT broker: ${this.brokerUrl}`);setTimeout(()=>{this.isConnected=!0;this.announcePresence();this.simulateMQTTDiscovery();this.dispatchEvent(new CustomEvent('connected'))},1500);return!0}catch(error){console.error('MQTT connection failed:',error);return!1}}
announcePresence(){console.log(`📢 Announcing presence on MQTT topic: ${this.topic}/${this.deviceId}/announce`);const announcement={deviceId:this.deviceId,peerId:this.deviceId,timestamp:Date.now(),capabilities:['webrtc','websocket'],namespace:this.namespace};console.log('📡 MQTT Announcement:',announcement)}
simulateMQTTDiscovery(){setInterval(()=>{if(Math.random()>0.6){const peerId=`mqtt-peer-${Date.now()}-${Math.random().toString(36).substr(2, 8)}`;this.dispatchEvent(new CustomEvent('peer-discovered',{detail:{peerId,discoveryMethod:'mqtt',endpoints:['websocket','webrtc'],metadata:{broker:this.brokerUrl,topic:this.topic,discovered:Date.now()}}}))}},15000)}
async disconnect(){this.isConnected=!1}}
class IPFSDiscoveryAdapter extends EventTarget{constructor(config){super();this.config=config;this.namespace=config.namespace||'auroradb';this.isConnected=!1}
async connect(){try{console.log(`🔗 Connecting to IPFS network with namespace: ${this.namespace}`);setTimeout(()=>{this.isConnected=!0;this.announceToIPFS();this.simulateIPFSDiscovery();this.dispatchEvent(new CustomEvent('connected'))},2000);return!0}catch(error){console.error('IPFS connection failed:',error);return!1}}
announceToIPFS(){console.log(`📢 Publishing to IPFS pubsub: /aurora-db/${this.namespace}/discovery`)}
simulateIPFSDiscovery(){setInterval(()=>{if(Math.random()>0.8){const peerId=`ipfs-peer-${Date.now()}-${Math.random().toString(36).substr(2, 8)}`;this.dispatchEvent(new CustomEvent('peer-discovered',{detail:{peerId,discoveryMethod:'ipfs',endpoints:['libp2p','webrtc'],metadata:{namespace:this.namespace,protocol:'libp2p',discovered:Date.now()}}}))}},20000)}
async disconnect(){this.isConnected=!1}}
class ActivityPubDiscoveryAdapter extends EventTarget{constructor(config){super();this.config=config;this.instanceUrl=config.instanceUrl;this.hashtag=config.hashtag||'#AuroraDB';this.isConnected=!1}
async connect(){try{console.log(`🔗 Connecting to ActivityPub instance: ${this.instanceUrl}`);console.log(`🏷️ Monitoring hashtag: ${this.hashtag}`);setTimeout(()=>{this.isConnected=!0;this.announceOnMastodon();this.simulateActivityPubDiscovery();this.dispatchEvent(new CustomEvent('connected'))},2500);return!0}catch(error){console.error('ActivityPub connection failed:',error);return!1}}
announceOnMastodon(){console.log(`📢 Posting to Mastodon with ${this.hashtag}`)}
simulateActivityPubDiscovery(){setInterval(()=>{if(Math.random()>0.9){const peerId=`actpub-peer-${Date.now()}-${Math.random().toString(36).substr(2, 8)}`;this.dispatchEvent(new CustomEvent('peer-discovered',{detail:{peerId,discoveryMethod:'activitypub',endpoints:['webrtc'],metadata:{instance:this.instanceUrl,hashtag:this.hashtag,discovered:Date.now()}}}))}},25000)}
async disconnect(){this.isConnected=!1}}
class DiscordDiscoveryAdapter extends EventTarget{constructor(config){super();this.config=config;this.channelId=config.channelId;this.guildId=config.guildId;this.isConnected=!1}
async connect(){try{console.log(`🔗 Connecting to Discord guild: ${this.guildId}, channel: ${this.channelId}`);setTimeout(()=>{this.isConnected=!0;this.announceOnDiscord();this.simulateDiscordDiscovery();this.dispatchEvent(new CustomEvent('connected'))},1800);return!0}catch(error){console.error('Discord connection failed:',error);return!1}}
announceOnDiscord(){console.log(`📢 Announcing on Discord channel: ${this.channelId}`)}
simulateDiscordDiscovery(){setInterval(()=>{if(Math.random()>0.75){const peerId=`discord-peer-${Date.now()}-${Math.random().toString(36).substr(2, 8)}`;this.dispatchEvent(new CustomEvent('peer-discovered',{detail:{peerId,discoveryMethod:'discord',endpoints:['webrtc'],metadata:{guild:this.guildId,channel:this.channelId,discovered:Date.now()}}}))}},12000)}
async disconnect(){this.isConnected=!1}}
class UniversalDiscoveryManager extends EventTarget{constructor(auroraDB){super();this.db=auroraDB;this.discoveryAdapters=new Map();this.peerRegistry=new Map();this.connectionAttempts=new Map();this.securityLayer=new SecureDiscoveryLayer();this.stats={peersDiscovered:0,connectionsAttempted:0,connectionsSuccessful:0,discoveryMethods:0}}
async initialize(config){console.log('🚀 Initializing Universal Discovery System...');if(config.security){await this.securityLayer.initializeSecurity(config.security)}
this.registerDiscoveryAdapters(config.methods||[])}
registerDiscoveryAdapters(methods){const adapterMap={'bittorrent-dht':BitTorrentDHTAdapter,'mqtt':MQTTDiscoveryAdapter,'ipfs':IPFSDiscoveryAdapter,'activitypub':ActivityPubDiscoveryAdapter,'discord':DiscordDiscoveryAdapter};methods.forEach(methodConfig=>{const AdapterClass=adapterMap[methodConfig.type];if(AdapterClass){const adapter=new AdapterClass(methodConfig.config||{});this.registerDiscoveryMethod(methodConfig.type,adapter)}})}
registerDiscoveryMethod(name,adapter){this.discoveryAdapters.set(name,adapter);this.stats.discoveryMethods++;adapter.addEventListener('peer-discovered',(event)=>{this.handlePeerDiscovery(name,event.detail)});adapter.addEventListener('connected',()=>{console.log(`✅ ${name} discovery adapter connected`)});console.log(`📡 Registered discovery method: ${name}`)}
async startDiscovery(methods=['all']){console.log('🔍 Starting peer discovery...');const adaptersToStart=methods.includes('all')?Array.from(this.discoveryAdapters.keys()):methods;const startPromises=adaptersToStart.map(async(method)=>{try{const adapter=this.discoveryAdapters.get(method);if(adapter){await adapter.connect();console.log(`✅ Started discovery via ${method}`)}}catch(error){console.warn(`❌ Failed to start ${method} discovery:`,error)}});await Promise.allSettled(startPromises);console.log(`🌐 Discovery started on ${adaptersToStart.length} networks`)}
async handlePeerDiscovery(discoveryMethod,peerInfo){const peerId=peerInfo.peerId;if(!this.securityLayer.validatePeer(peerId,discoveryMethod)){console.warn(`🚫 Peer validation failed for ${peerId} via ${discoveryMethod}`);return}
if(peerInfo.encrypted){const decryptedInfo=await this.securityLayer.decryptDiscoveryData(peerInfo);if(!decryptedInfo){console.warn(`🔐 Failed to decrypt peer info for ${peerId}`);return}
Object.assign(peerInfo,decryptedInfo)}
this.stats.peersDiscovered++;if(!this.peerRegistry.has(peerId)){console.log(`🆕 Discovered new peer: ${peerId.substring(0, 12)}... via ${discoveryMethod}`);this.peerRegistry.set(peerId,{...peerInfo,discoveryMethods:[discoveryMethod],firstSeen:Date.now(),connectionAttempts:0,connected:!1})}else{const existing=this.peerRegistry.get(peerId);if(!existing.discoveryMethods.includes(discoveryMethod)){existing.discoveryMethods.push(discoveryMethod);console.log(`🔄 Peer ${peerId.substring(0, 12)}... now discoverable via ${discoveryMethod}`)}
existing.lastSeen=Date.now()}
if(!this.db.peer?.connections?.has(peerId)){this.attemptConnection(peerId)}
this.dispatchEvent(new CustomEvent('peer-discovered',{detail:{peerId,discoveryMethod,peerInfo}}))}
async attemptConnection(peerId){const peerInfo=this.peerRegistry.get(peerId);if(peerInfo.connectionAttempts>=3){console.log(`⏭️ Max connection attempts reached for ${peerId.substring(0, 12)}...`);return}
peerInfo.connectionAttempts++;this.stats.connectionsAttempted++;console.log(`🔗 Attempting connection to ${peerId.substring(0, 12)}... (attempt ${peerInfo.connectionAttempts}/3)`);try{if(this.db.peer&&typeof this.db.peer.connectToPeer==='function'){const connection=await this.db.peer.connectToPeer(peerId);if(connection){peerInfo.connected=!0;this.stats.connectionsSuccessful++;console.log(`✅ Successfully connected to ${peerId.substring(0, 12)}...`);this.dispatchEvent(new CustomEvent('peer-connected',{detail:{peerId,connection}}));return}}
await this.tryFallbackConnection(peerId,peerInfo)}catch(error){console.warn(`❌ Connection failed for ${peerId.substring(0, 12)}...:`,error.message)}}
async tryFallbackConnection(peerId,peerInfo){const methods=['webrtc-fallback','websocket-fallback','signaling-server'];for(const method of methods){try{console.log(`🔄 Trying ${method} connection to ${peerId.substring(0, 12)}...`);if(Math.random()>0.7){peerInfo.connected=!0;peerInfo.connectionMethod=method;this.stats.connectionsSuccessful++;console.log(`✅ Connected to ${peerId.substring(0, 12)}... via ${method}`);this.dispatchEvent(new CustomEvent('peer-connected',{detail:{peerId,method}}));return!0}}catch(error){console.warn(`${method} failed:`,error)}}
return!1}
getDiscoveryStats(){return{...this.stats,activeMethods:Array.from(this.discoveryAdapters.keys()),discoveredPeers:this.peerRegistry.size,connectedPeers:Array.from(this.peerRegistry.values()).filter(p=>p.connected).length,peerSources:this.getPeerSourceBreakdown(),uptime:Date.now()-(this.startTime||Date.now())}}
getPeerSourceBreakdown(){const breakdown={};for(const[peerId,info]of this.peerRegistry){for(const method of info.discoveryMethods){breakdown[method]=(breakdown[method]||0)+1}}
return breakdown}
async stopDiscovery(){console.log('🛑 Stopping discovery...');for(const[name,adapter]of this.discoveryAdapters){try{await adapter.disconnect();console.log(`✅ Stopped ${name} discovery`)}catch(error){console.warn(`❌ Error stopping ${name}:`,error)}}}}
class P2PAdapter extends EventTarget{constructor(config){super();this.config=config;this.id=config.id||this.generateId();this.connections=new Map();this.isConnected=!1}
...
class UniversalP2PAdapter extends P2PAdapter{constructor(config){super(config);this.primaryAdapter=null;this.discoveryManager=null}
async connect(){try{const PrimaryAdapterClass=this.config.primaryAdapter==='webrtc'?WebRTCAdapter:PeerJSAdapter;this.primaryAdapter=new PrimaryAdapterClass(this.config);await this.primaryAdapter.connect();this.setupEventForwarding();this.isConnected=!0;this.emit('open',this.id);return this.id}catch(error){console.error('Universal adapter connection failed:',error);throw error}}
setupEventForwarding(){['connection','message','disconnect','close','error'].forEach(eventName=>{this.primaryAdapter.addEventListener(eventName,(event)=>{this.dispatchEvent(new CustomEvent(eventName,{detail:event.detail}))})})}
async listPeers(){return this.primaryAdapter?this.primaryAdapter.listPeers():[]}
async connectToPeer(peerId){return this.primaryAdapter?this.primaryAdapter.connectToPeer(peerId):null}
send(peerId,data){return this.primaryAdapter?this.primaryAdapter.send(peerId,data):!1}
broadcast(data){return this.primaryAdapter?this.primaryAdapter.broadcast(data):0}
async disconnect(){if(this.primaryAdapter){await this.primaryAdapter.disconnect()}
this.isConnected=!1}
async destroy(){if(this.primaryAdapter){await this.primaryAdapter.destroy()}
this.isConnected=!1}
getConnectionCount(){return this.primaryAdapter?this.primaryAdapter.getConnectionCount():0}}
...
class WebRTCAdapter extends P2PAdapter{constructor(config){super(config);this.signalingServer=config.signalingServer||'wss://signaling.example.com';this.rtcConfig=config.rtcConfig||{iceServers:[{urls:'stun:stun.l.google.com:19302'}]};this.socket=null;this.peerConnections=new Map()}
...
registerDefaults(){this.register('peerjs',PeerJSAdapter);this.register('webrtc',WebRTCAdapter);this.register('universal',UniversalP2PAdapter)}
...
class PeerStore extends IDBStore{constructor(scheme){super(scheme);this.p2pAdapter=scheme.p2pAdapter||'peerjs';this.p2pConfig={id:scheme.id,server:scheme.server||{},connectionTimeout:scheme.connectionTimeout||15000,peerConnectionTimeout:scheme.peerConnectionTimeout||10000,maxReconnectAttempts:scheme.maxReconnectAttempts||5,reconnectDelay:scheme.reconnectDelay||1000,...scheme.p2pConfig};this.syncStrategy=scheme.syncStrategy||'timestamp';this.autoSync=scheme.autoSync!==!1;this.syncInterval=scheme.syncInterval||30000;this.peer=null;this.discoveryManager=null;this.syncTimer=null;this.isConnected=!1;this.lastSyncTime=0;this.net=this.createNetAdapter();this.initializePeer()}
async initializePeer(){try{this.peer=p2pRegistry.create(this.p2pAdapter,this.p2pConfig);if(this.p2pAdapter==='universal'&&this.p2pConfig.discoveryConfig){this.discoveryManager=new UniversalDiscoveryManager(this);await this.discoveryManager.initialize(this.p2pConfig.discoveryConfig)}
this.setupPeerHandlers();await this.peer.connect();this.isConnected=!0;if(this.discoveryManager){await this.discoveryManager.startDiscovery()}else{await this.connectToPeers()}
if(this.autoSync){this.startAutoSync()}}catch(error){this.dispatchEvent(new CustomEvent('error',{detail:error}));throw error}}
setupPeerHandlers(){this.peer.addEventListener('open',(event)=>{console.log(`🌐 Peer connected with ID: ${event.detail}`);this.dispatchEvent(new CustomEvent('open',{detail:{peerId:event.detail}}))});this.peer.addEventListener('connection',(event)=>{console.log(`🤝 New peer connected: ${event.detail.peerId}`);this.dispatchEvent(new CustomEvent('connection',{detail:event.detail}))});this.peer.addEventListener('message',(event)=>{this.handleMessage(event.detail.data,event.detail.from,event.detail.connection)});this.peer.addEventListener('disconnect',()=>{this.isConnected=!1;this.dispatchEvent(new CustomEvent('disconnect'))});this.peer.addEventListener('close',()=>{this.isConnected=!1;this.dispatchEvent(new CustomEvent('close'))});this.peer.addEventListener('error',(event)=>{this.dispatchEvent(new CustomEvent('error',{detail:event.detail}))});if(this.discoveryManager){this.discoveryManager.addEventListener('peer-discovered',(event)=>{console.log(`🔍 Peer discovered via ${event.detail.discoveryMethod}: ${event.detail.peerId.substring(0, 12)}...`)});this.discoveryManager.addEventListener('peer-connected',(event)=>{console.log(`✅ Connected to discovered peer: ${event.detail.peerId.substring(0, 12)}...`)})}}
...
this.__channels__=this.__channels__.filter(ch=>ch.header!==header);this.__channels__.push({header,methods})},getConnectionCount(){return self.peer?self.peer.getConnectionCount():0},isConnected(){return self.isConnected},getPeerId(){return self.peer?self.peer.getId():null},getAdapter(){return self.p2pAdapter},getDiscoveryStats(){return self.discoveryManager?self.discoveryManager.getDiscoveryStats():null}}}
...
async destroy(){this.stopAutoSync();if(this.discoveryManager){await this.discoveryManager.stopDiscovery()}
if(this.peer){await this.peer.destroy()}
await super.close()}}
...
getDiscoveryStats(){return this.connected?this.db.net.getDiscoveryStats():null}
getScheme(){return{...this.scheme}}
getStats(){return{name:this.scheme.name,isReady:this.isReady,isDistributed:this.connected,p2pAdapter:this.getP2PAdapter(),peerId:this.getPeerId(),connections:this.getConnectionCount(),discovery:this.getDiscoveryStats(),scheme:this.getScheme()}}}
AuroraDB.P2PRegistry=p2pRegistry;AuroraDB.P2PAdapter=P2PAdapter;AuroraDB.UniversalDiscoveryManager=UniversalDiscoveryManager;AuroraDB.SecureDiscoveryLayer=SecureDiscoveryLayer;return AuroraDB})()